...
module.exports = class Task {
	...
	repeatTask() {
		let { name, axiosOptions, cookies, fakeIP, handler } = this

		axiosOptions = Object.assign({}, axiosOptions)
		cookies = Object.assign({}, cookies)

		this.counter++
		return new Task({ name, axiosOptions, cookies, fakeIP, handler })
	}
	generateTask() {
		if (!this.functional) {
			return this.copy()
		...
		for (let key in this.paramSetters) {
			...
			url = url.replace(pattern + key, params[key])
		...
		let task = new Task({ name, axiosOptions, cookies, fakeIP, handler })
